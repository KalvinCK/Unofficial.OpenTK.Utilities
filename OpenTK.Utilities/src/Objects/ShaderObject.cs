...
public class ShaderObject : IReadOnlyShaderObject, IDisposable
{
    public ShaderObject(params IReadOnlyShaderSource[] shaders)
    {
    ...
    public ShaderObject(bool separable, params IReadOnlyShaderSource[] shaders)
    {
    ...
    private ShaderObject(int programID, List<ShaderType> composion, bool separable)
    {
    ...
    public static ShaderObject CreateProgram(params IReadOnlyShaderSource[] shaders)
    {
        return new ShaderObject(Create(false, shaders), shaders.Select(i => i.Type).ToList(), false);
    }

    public static ShaderObject CreateProgramSeparable(params IReadOnlyShaderSource[] shaders)
    {
        return new ShaderObject(Create(true, shaders), shaders.Select(i => i.Type).ToList(), true);
    }
    ...
    {
        return $"Label: {(string.IsNullOrEmpty(this.Label) ? @"''" : this.Label)} Separable: {this.Separable} Composion: {this.Composion.Count} Uniforms: {this.ActivesUniforms.Count} Attributes: {this.ActivesAttributes.Count}";
    }
    ...
    private static int Create(bool separable, params IReadOnlyShaderSource[] shadersSource)
    {
        if (shadersSource.Length == 0)
        {
            throw new ShaderProgramException("No shaders were provided");
        }
