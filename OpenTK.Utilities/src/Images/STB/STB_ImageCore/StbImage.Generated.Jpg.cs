...
namespace OpenTK.Utilities.Images
{
	unsafe partial class ImageCore
	{
		...
		public static byte[] JpegDezigzag =
		{
		...
		public static int JpegTest(Context s)
		{
			var r = 0;
			var j = new Jpeg();
			if (j == null)
			...
			j.s = s;
			SetupJpeg(j);
			r = DecodeJpegHeader(j, STBI__SCAN_type);
			Rewind(s);
			return r;
		}

		public static void* JpegLoad(Context s, int* x, int* y, int* comp, int req_comp,
			ResultInfo* ri)
		{
			byte* result;
			var j = new Jpeg();
		...
		public static int JpegInfo(Context s, int* x, int* y, int* comp)
		{
			var result = 0;
			var j = new Jpeg();
			if (j == null)
				return Err("outofmem");
			j.s = s;
			result = JpegInfoRaw(j, x, y, comp);
			return result;
		}

		public static int BuildHuffman(Huffman* h, int* count)
		{
			var i = 0;
			var j = 0;
			var k = 0;
		...
		public static void GrowBufferUnsafe(Jpeg j)
		{
			do
			{
		...
		public static int JpegHuffDecode(Jpeg j, Huffman* h)
		{
			uint temp = 0;
			var c = 0;
			var k = 0;
		...
		public static int ExtendReceive(Jpeg j, int n)
		{
			uint k = 0;
			var sgn = 0;
			if (j.code_bits < n)
		...
		public static int JpegGetBits(Jpeg j, int n)
		{
			uint k = 0;
			if (j.code_bits < n)
				GrowBufferUnsafe(j);
		...
		public static int JpegGetBit(Jpeg j)
		{
			uint k = 0;
			if (j.code_bits < 1)
				GrowBufferUnsafe(j);
		...
		public static int JpegDecodeBlock(Jpeg j, short* data, Huffman* hdc, Huffman* hac,
		...
		public static int JpegDecodeBlockProgDc(Jpeg j, short* data, Huffman* hdc, int b)
		{
			var diff = 0;
			var dc = 0;
			var t = 0;
		...
		public static int JpegDecodeBlockProgAc(Jpeg j, short* data, Huffman* hac, short[] fac)
		{
			var k = 0;
			if (j.spec_start == 0)
				return Err("can't merge dc and ac");
				...
				do
				{
					...
					else
					{
						...
						else
						{
							k += r;
							zig = JpegDezigzag[k++];
							data[zig] = (short)(ExtendReceive(j, s) * (1 << shift));
						}
					}
				} while (k <= j.spec_end);
			}
		...
		public static int ParseEntropyCodedData(Jpeg z)
		{
			JpegReset(z);
			if (z.progressive == 0)
			{
				if (z.scan_n == 1)
				{
					...
					for (j = 0; j < h; ++j)
						for (i = 0; i < w; ++i)
						{
							...
							fixed (Huffman* dptr = &z.huff_dc[z.img_comp[n].hd])
							fixed (Huffman* aptr = &z.huff_ac[ha])
							{
								if (JpegDecodeBlock(z, data, dptr, aptr,
				...
				for (j = 0; j < h; ++j)
					for (i = 0; i < w; ++i)
					{
						...
						if (z.spec_start == 0)
						{
							fixed (Huffman* dptr = &z.huff_dc[z.img_comp[n].hd])
							{
								if (JpegDecodeBlockProgDc(z, data, dptr, n) == 0)
									return 0;
							}
						}
						else
						{
							...
							fixed (Huffman* aptr = &z.huff_ac[ha])
							{
								if (JpegDecodeBlockProgAc(z, data, aptr, z.fast_ac[ha]) == 0)
									return 0;
							}
						}
		...
		public static void JpegDequantize(short* data, ushort[] dequant)
		{
			var i = 0;
			for (i = 0; i < 64; ++i)
			...
			if (z.progressive != 0)
			{
				...
				for (n = 0; n < z.s.img_n; ++n)
				{
					...
					for (j = 0; j < h; ++j)
						...
						{
							var data = z.img_comp[n].coeff + 64 * (i + j * z.img_comp[n].coeff_w);
							JpegDequantize(data, z.dequant[z.img_comp[n].tq]);
							z.idct_block_kernel(z.img_comp[n].data + z.img_comp[n].w2 * j * 8 + i * 8, z.img_comp[n].w2,
								data);
						}
				}
		...
		public static int ProcessMarker(Jpeg z, int m)
		{
			var L = 0;
			switch (m)
			{
				...
				case 0xC4:
					...
					while (L > 0)
					{
						...
						if (tc == 0)
						{
							fixed (Huffman* hptr = &z.huff_dc[th])
							{
								if (BuildHuffman(hptr, sizes) == 0)
									return 0;
						...
						{
							fixed (Huffman* aptr = &z.huff_ac[th])
							{
								if (BuildHuffman(aptr, sizes) == 0)
									return 0;
			...
			if (m >= 0xE0 && m <= 0xEF || m == 0xFE)
			{
				...
				if (m == 0xE0 && L >= 5)
				{
					...
					var i = 0;
					for (i = 0; i < 5; ++i)
						if (Get8(z.s) != ProcessMarkerTag[i])
							ok = 0;
					L -= 5;
					if (ok != 0)
						z.jfif = 1;
		...
		public static int ProcessScanHeader(Jpeg z)
		{
			var i = 0;
			var Ls = Get16be(z.s);
			z.scan_n = Get8(z.s);
		...
		public static int FreeJpegComponents(Jpeg z, int ncomp, int why)
		{
			var i = 0;
			for (i = 0; i < ncomp; ++i)
			{
		...
		public static int ProcessFrameHeader(Jpeg z, int scan)
		{
			var s = z.s;
			var Lf = 0;
			var p = 0;
			...
			for (i = 0; i < s.img_n; ++i)
			{
				...
				if (s.img_n == 3 && z.img_comp[i].id == ProcessFrameHeaderRgb[i])
					++z.rgb;
				q = Get8(s);
				z.img_comp[i].h = q >> 4;
			...
			if (scan != STBI__SCAN_load)
				return 1;
			if (Mad3sizesValid((int)s.img_x, (int)s.img_y, s.img_n, 0) == 0)
				return Err("too large");
			for (i = 0; i < s.img_n; ++i)
			{
			...
			for (i = 0; i < s.img_n; ++i)
			{
				...
				z.img_comp[i].raw_coeff = null;
				z.img_comp[i].linebuf = null;
				z.img_comp[i].raw_data = MallocMad2(z.img_comp[i].w2, z.img_comp[i].h2, 15);
				if (z.img_comp[i].raw_data == null)
					return FreeJpegComponents(z, i + 1, Err("outofmem"));
				z.img_comp[i].data = (byte*)(((long)z.img_comp[i].raw_data + 15) & ~15);
				if (z.progressive != 0)
				{
					z.img_comp[i].coeff_w = z.img_comp[i].w2 / 8;
					z.img_comp[i].coeff_h = z.img_comp[i].h2 / 8;
					z.img_comp[i].raw_coeff = MallocMad3(z.img_comp[i].w2, z.img_comp[i].h2, sizeof(short), 15);
					if (z.img_comp[i].raw_coeff == null)
						return FreeJpegComponents(z, i + 1, Err("outofmem"));
		...
		public static int DecodeJpegHeader(Jpeg z, int scan)
		{
			var m = 0;
			z.jfif = 0;
			...
			if (ProcessFrameHeader(z, scan) == 0)
				return 0;
			return 1;
		}

		public static int DecodeJpegImage(Jpeg j)
		{
			var m = 0;
			for (m = 0; m < 4; m++)
			{
			...
			if (DecodeJpegHeader(j, STBI__SCAN_load) == 0)
				return 0;
			m = GetMarker(j);
			while (!(m == 0xd9))
			{
				if (m == 0xda)
				{
					if (ProcessScanHeader(j) == 0)
						return 0;
					if (ParseEntropyCodedData(j) == 0)
						return 0;
					if (j.marker == 0xff)
						while (AtEof(j.s) == 0)
						{
				...
				{
					var Ld = Get16be(j.s);
					var NL = (uint)Get16be(j.s);
					if (Ld != 4)
						return Err("bad DNL len");
					if (NL != j.s.img_y)
						return Err("bad DNL height");
				}
				else
				{
		...
		{
			j.idct_block_kernel = IdctBlock;
			j.YCbCr_to_RGB_kernel = YCbCrToRGBRow;
			j.resample_row_hv_2_kernel = ResampleRowHv2;
		}

		public static void CleanupJpeg(Jpeg j)
		{
			FreeJpegComponents(j, j.s.img_n, 0);
		}
		...
		public static byte* load_jpeg_image(Jpeg z, int* out_x, int* out_y, int* comp, int req_comp)
		{
...

			{
				...
				res_comp[0] = new Resample();
				res_comp[1] = new Resample();
				res_comp[2] = new Resample();
				res_comp[3] = new Resample();
				...
				output = (byte*)MallocMad3(n, (int)z.s.img_x, (int)z.s.img_y, 1);
				if (output == null)
				{
					CleanupJpeg(z);
					return (byte*)(ulong)(Err("outofmem") != 0 ? 0 : 0);
				...
				for (j = 0; j < z.s.img_y; ++j)
				{
					...
					else
					{
						if (is_rgb != 0)
						{
							if (n == 1)
								for (i = 0; i < z.s.img_x; ++i)
									*_out_++ = ComputeY(coutput[0][i], coutput[1][i], coutput[2][i]);
							else
								for (i = 0; i < z.s.img_x; ++i, _out_ += 2)
								{
									_out_[0] = ComputeY(coutput[0][i], coutput[1][i], coutput[2][i]);
									_out_[1] = 255;
								}
						}
		...
		public static int JpegInfoRaw(Jpeg j, int* x, int* y, int* comp)
		{
			if (DecodeJpegHeader(j, STBI__SCAN_header) == 0)
			{
				Rewind(j.s);
				return 0;
			}
		...
		public class Jpeg
		{
			...
			public Huffman[] huff_ac = new Huffman[4];
			public Huffman[] huff_dc = new Huffman[4];
