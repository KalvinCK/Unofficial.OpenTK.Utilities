...
{
	public unsafe partial class ImageWriteCore
	{
		public static int NativeAllocations => MemoryStats.Allocations;

		public static void Writefv(WriteContext s, string fmt, params object[] v)
		{
			var vindex = 0;
		...
		public static void Writef(WriteContext s, string fmt, params object[] v)
		{
			Writefv(s, fmt, v);
		}

		public static int Outfile(WriteContext s, int rgb_dir, int vdir, int x, int y, int comp,
		...
		public static int WriteHdrCore(WriteContext s, int x, int y, int comp, float* data)
		{
			...
			for (i = 0; i < y; i++)
			{
				WriteHdrScanline(s, x, comp, scratch, data + comp*i*x);
			}
